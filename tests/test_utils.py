...
def test_card_info(start: date, end: date, expected: List[Dict[str, Any]]) -> None:
    """Тестирование корректного вывода результатов с номером карты, тратами и описанием"""
    result: List[Dict[str, Any]]= card_info(trans_list, start, end)
...
def test_sort_by_amount(start: date, end: date, reverse_str: bool, expected: List[Dict[str, Any]]) -> None:
